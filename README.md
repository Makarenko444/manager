...
Ниже описаны два варианта: через Docker и через синхронизацию статических файлов. Выберите тот, который удобнее в вашей инфраструктуре.

### Вариант 1. Развертывание в Docker

1. **Подготовьте сервер.** Убедитесь, что на нём установлены Docker и Docker Compose v2.
2. **Скопируйте проект.** Передайте каталог `manager` на сервер с помощью `git clone`, `scp` или `rsync`.
3. **Перейдите в папку проекта** на сервере:
   ```bash
   cd /path/to/manager
   ```
...
5. **Проверьте, что контейнер работает:**
   ...
   docker compose ps
   ```
6. **Настройте проксирование.** По умолчанию Nginx в контейнере слушает порт 80 и проброшен на 8080 хоста. Настройте брандмауэр и, при необходимости, внешний веб-сервер (например, основной Nginx на хосте), чтобы проксировать запросы на контейнер.
7. **Проверьте сайт.** Перейдите в браузере по адресу `http://<ваш_сервер>:8080` или по доменному имени, если настроен прокси.
